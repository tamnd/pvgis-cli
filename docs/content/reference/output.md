...
describes what users get. Pick a format with `-o`, or let pvgis choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pvgis <command> -o table   # aligned columns for reading
pvgis <command> -o jsonl   # one JSON object per line, for piping
pvgis <command> -o json    # a single JSON array
pvgis <command> -o csv     # spreadsheet friendly
pvgis <command> -o tsv     # tab-separated
pvgis <command> -o url     # just the URL column
pvgis <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pvgis <command> --fields id,title,url
```
...
```bash
pvgis <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pvgis <command>            # a table, because this is a terminal
pvgis <command> | wc -l    # JSONL, because this is a pipe
```
