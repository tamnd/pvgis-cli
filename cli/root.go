// Package cli assembles the pvgis command tree from the pvgis
...
// NewApp assembles the kit application from the pvgis domain. The
...
//
// To add a command, declare it in pvgis/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := pvgis.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(pvgis.Domain{}).Register(app)
